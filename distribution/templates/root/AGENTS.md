...
**BLOCKED 工具默认 T2（静默降级，但不吞用户 T3 意图）**：实际执行档位 = MIN(用户指定档位(默认=变更类默认), 当前工具最高支持档位)。BLOCKED 工具最高 T2，故**默认实际执行 T2——静默以 T2 执行，不弹拦截**。但**静默降级仅当用户未表达流程意图时**；若用户消息含“需求/草案/设计/评审/T3”任一词，或变更是**新功能/新协议/新存储/用户点名需求**（不是已有行为缺陷修复），**必须提示**：“`[SOP 拦截] 当前工具最高 T2，该请求需要 T3。建议：切 Claude Code/Cursor，或回复‘仍按 T2 实现’。”**已有行为的单点逻辑修复保持静默 T2，不拦截。
