...
- **内部持久化存储字段必须简写**：仅限 Redis 对象字段、Mongo 对象字段、其他对象不要简写。规则源 `.ai-workspace/context/coding-style.md`（与 design-architect / design-reviewer / implementation-engine **同一文件**）。若实现与 06 字面一致但存储键未按该文件简写：**不得因「符合契约」判 PASS**；标 `[DESIGN_FLAW: plan-mandated]`，阻断并回设计门禁。 
