...
## 执行纪律（自包含）
本角色**只遵守本 Skill 正文**。不要去读 Superpowers 插件目录，也不要读 `implementer-prompt.md` / `test-driven-development/SKILL.md` 等上游全文——那些是流程/派发材料，与本角色规则冲突或已内联于本节。T3 流程由 controller 显式调用 Superpowers 流程 skill，不由本角色加载。
