...
5. 检查每个 `TC-*` 的前置条件、触发步骤、断言和清理是否可由另一个 AI 直接执行。
6. 检查四层断言是否都使用 `{ target, operator, expected }`，或以 `operator=N_A` 给出明确不适用原因；拒绝空断言层和“正确”“正常”“符合预期”等自由文本结论。
7. 检查 GM 只承担 Arrange/Observe/Cleanup，核心 Act 使用正式协议或正式业务入口。
8. 检查 Case 是否存在重复、伪拆分、不可达前置、相互污染，或大量组合未覆盖关键决策结果。
